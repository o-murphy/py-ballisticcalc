...
class TestTsitouras:
    ...
    def test_public_api_defaults_and_round_trip(self):
        engine = self.exts.CythonizedTsitourasIntegrationEngine({
        ...
        assert self.exts.CythonizedTsitourasIntegrationEngine({}).relative_tolerance == 1e-6
    ...
    def test_invalid_tolerances(self, key, value):
        with pytest.raises(ValueError):
            self.exts.CythonizedTsitourasIntegrationEngine({key: value})

    def test_tighter_tolerances_increase_work(self):
        loose = self.exts.CythonizedTsitourasIntegrationEngine({"relative_tolerance": 1e-5})
        tight = self.exts.CythonizedTsitourasIntegrationEngine({"relative_tolerance": 1e-8})
        _integrate(loose); _integrate(tight)
        assert tight.get_step_stats()[0] > loose.get_step_stats()[0]

        atol_loose = self.exts.CythonizedTsitourasIntegrationEngine(
            {"relative_tolerance": 1e-12, "absolute_tolerance": 1e-2})
        atol_tight = self.exts.CythonizedTsitourasIntegrationEngine(
    ...
    def test_trajectory_is_within_conservative_rk4_bound(self):
        ...
        actual = _integrate(self.exts.CythonizedTsitourasIntegrationEngine({}))
    ...
    def test_matches_scipy_rk45_when_available(self):
        ...
        tsit = _integrate(self.exts.CythonizedTsitourasIntegrationEngine({}))
        scipy = _integrate(SciPyIntegrationEngine({
            "integration_method": "RK45", "relative_tolerance": 1e-6,
            "absolute_tolerance": 1e-6,
        }))
        assert max(abs(a.height.raw_value - b.height.raw_value)
                   for a, b in zip(tsit.samples, scipy.samples)) < .5
        assert max(abs(a.velocity.raw_value - b.velocity.raw_value)
                   for a, b in zip(tsit.samples, scipy.samples)) < .1
    ...
    def test_wind_transition_has_no_stale_fsal_derivative(self):
        ...
        tsit = self.exts.CythonizedTsitourasIntegrationEngine({})
        result = tsit.integrate(shot, Distance.Meter(1000), Distance.Meter(50),
                                 0., TrajFlag.ALL, dense_output=True)
        ...
        assert tsit.get_step_stats()[0] > 0
