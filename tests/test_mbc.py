...
class TestMBC:
    ...
    def test_mbc3(self):
        ...
        dm_multi = DragModelMultiBC(
            [BCPoint(.5, V=self.baseline_trajectory[3].velocity), BCPoint(.22, V=self.baseline_trajectory[2].velocity)],
            TableG7)
        multi_shot = Shot(weapon=self.weapon, ammo=Ammo(dm_multi, self.ammo.mv))
        multi_trajectory = self.calc.fire(shot=multi_shot, trajectory_range=self.range,
                                          trajectory_step=self.step).trajectory
        # Should show no change before 200 yards
        assert pytest.approx(multi_trajectory[1].velocity.raw_value, abs=1e-5) == self.baseline_trajectory[
    ...
    def test_mbc(self):
        dm = DragModelMultiBC([BCPoint(0.275, V=Velocity.MPS(800)), BCPoint(0.255, V=Velocity.MPS(500)),
