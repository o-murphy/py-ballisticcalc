...
# Define dependencies using a dictionary
# Keys are extension names, values are lists of C source files they depend on
deps = {
    "base_engine": [v3d_c_source, helpers_c_source],
    "euler_engine": [v3d_c_source],
    "rk4_engine": [v3d_c_source],
    # Add other extensions and their C dependencies here if they exist
}

# *** ЗМІНА ТУТ: Додано 'include' до include_dirs для пошуку v3d.h ***
include_dirs = [
    ext_base_dir,  # Для .pxd файлів (e.g., py_ballisticcalc_exts/cy_bindings.pxd)
    os.path.join(ext_base_dir, 'src'),  # Для .h файлів, якщо вони є у src/
...
for name in extension_names:
    ...
    # Check the deps dictionary for additional C source dependencies
    if name in deps:
        sources.extend(deps[name])
    ...
    extensions.append(
        Extension(
            ...
            # extra_compile_args=["-std=c99"], # Uncomment if specific C standard is required
            # libraries=['m'] # For Linux/macOS, add 'm' for math functions. For Windows, usually not needed.
        )
    )
