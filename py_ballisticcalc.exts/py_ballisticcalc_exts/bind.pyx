...
cdef BCLIBC_ShotProps BCLIBC_ShotProps_from_pyobject(object shot_info, double calc_step = 1.0):
    ...
    cdef size_t n_winds = len(shot_info.winds)
    cdef vector[BCLIBC_Wind] winds_vec
    winds_vec.reserve(n_winds)
    for w in shot_info.winds:
        winds_vec.emplace_back(
            <double>w.velocity._fps,
            <double>w.direction_from._rad,
            <double>w.until_distance._feet,
    ...
    shot.bc = shot_info.ammo.dm.BC
    shot.weight_grain = shot_info.ammo.dm.weight._grain
    shot.diameter_inch = shot_info.ammo.dm.diameter._inch
    shot.length_inch = shot_info.ammo.dm.length._inch
    ...
    shot.sight_height_ft = shot_info.weapon.sight_height._feet
    shot.twist_inch = shot_info.weapon.twist._inch
    shot.temp_c = shot_info.atmo._t0
    shot.pressure_hpa = 0.0 if shot_info.atmo.density_ratio == 0.0 else shot_info.atmo._p0
    shot.altitude_ft = shot_info.atmo._a0
    shot.humidity = shot_info.atmo.humidity
