...
void ShotData_t_free(ShotData_t *shot_data_ptr) {
    if (shot_data_ptr == NULL) return;

    Curve_t_free(&shot_data_ptr->curve);
    MachList_t_free(&shot_data_ptr->mach_list);
}
 ...
 *     0.076474 * π / (4 * 2 * 144)
 *
 * @param shot_data_ptr Pointer to the ShotData_t structure containing BC, drag curve, and Mach list.
