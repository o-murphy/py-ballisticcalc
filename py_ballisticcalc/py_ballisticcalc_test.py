...
class TestPyBallisticCalc(unittest.TestCase):
    ...
    def validate_one(self, data: TrajectoryData, distance: float, velocity: float, mach: float, energy: float,
        ...
        if distance > 1:
            self.assertEqualCustom(path, data.drop.get_in(adjustment_unit), 0.5, 'Hold')
        ...
        if distance > 1:
            self.assertEqualCustom(windage, data.windage_adjustment.get_in(adjustment_unit), 0.5, "WAdj")
    ...
    def test_path_g1(self):
        ...
        weapon = Weapon(unit.Distance(2, unit.DistanceInch).must_create(), zero)
        atmosphere = Atmosphere()
        shot_info = ShotParameters(unit.Angular(0.001228, unit.AngularRadian).must_create(),
                                   unit.Distance(1000, unit.DistanceYard).must_create(),
                                   unit.Distance(100, unit.DistanceYard).must_create())
        wind = WindInfo.create_only_wind_info(unit.Velocity(5, unit.VelocityMPH).must_create(),
                                              unit.Angular(-45, unit.AngularDegree).must_create())
    ...
    def test_path_g7(self):
        bc = BallisticCoefficient(0.223, DragTableG7)
        projectile = ProjectileWithDimensions(bc, unit.Distance(0.308, unit.DistanceInch).must_create(),
                                              unit.Distance(1.282, unit.DistanceInch).must_create(),
                                              unit.Weight(168, unit.WeightGrain).must_create())
        ammo = Ammunition(projectile, unit.Velocity(2750, unit.VelocityFPS).must_create())
        zero = ZeroInfo(unit.Distance(100, unit.DistanceYard).must_create())
        twist = TwistInfo(TwistRight, unit.Distance(11.24, unit.DistanceInch).must_create())
        weapon = Weapon.create_with_twist(unit.Distance(2, unit.DistanceInch).must_create(), zero, twist)
        atmosphere = Atmosphere()
        shot_info = ShotParameters(unit.Angular(4.221, unit.AngularMOA).must_create(),
                                   unit.Distance(1000, unit.DistanceYard).must_create(),
                                   unit.Distance(100, unit.DistanceYard).must_create())
        wind = WindInfo.create_only_wind_info(unit.Velocity(5, unit.VelocityMPH).must_create(),
                                              unit.Angular(-45, unit.AngularDegree).must_create())
