from libc.math cimport fabs, pow, sin, cos, atan, log10, floor
from py_ballisticcalc.bmath.cunit import *
from py_ballisticcalc.bmath.cvector import Vector
from py_ballisticcalc.extended.bin.projectile import Ammunition
from py_ballisticcalc.extended.bin.weapon import Weapon, TwistLeft
from py_ballisticcalc.extended.bin.atmosphere import Atmosphere
from py_ballisticcalc.extended.bin.shot_parameters import ShotParameters
from py_ballisticcalc.extended.bin.wind import WindInfo
from py_ballisticcalc.extended.bin.trajectory_data import TrajectoryData, Timespan
...
cdef class TrajectoryCalculator:
    ...
    cpdef sight_angle(self, ammunition: Ammunition, weapon: Weapon, atmosphere: Atmosphere):
        cdef float calculation_step = self.get_calculation_step(
            Distance(10, weapon.zero().zero_distance().units()).get_in(DistanceFoot))

        cdef float mach = atmosphere.mach().get_in(VelocityFPS)
        cdef float density_factor = atmosphere.density_factor()
        cdef float muzzle_velocity = ammunition.muzzle_velocity().get_in(VelocityFPS)
        cdef float barrel_azimuth = 0.0
        cdef float barrel_elevation = 0

        cdef int iterations_count = 0

        cdef float zero_finding_error = cZeroFindingAccuracy * 2
        cdef gravity_vector = Vector(0, cGravityConstant, 0)
        cdef float velocity
        cdef float time

        cdef range_vector
        cdef velocity_vector

        cdef float zero_distance
        cdef float maximum_range
        cdef float delta_time
        cdef float drag
        cdef delta_range_vector
        ...
        while zero_finding_error > cZeroFindingAccuracy and iterations_count < cMaxIterations:
            ...
            range_vector = Vector(0.0, -weapon.sight_height().get_in(DistanceFoot), 0)
            ...
            zero_distance: float = weapon.zero().zero_distance().get_in(DistanceFoot)
    ...
    cpdef trajectory(self, ammunition: Ammunition, weapon: Weapon, atmosphere: Atmosphere,
        ...
        cdef float range_to = shot_info.maximum_distance().get_in(DistanceFoot)
        cdef float step = shot_info.step().get_in(DistanceFoot)

        cdef float calculation_step = self.get_calculation_step(step)

        cdef float bullet_weight = ammunition.bullet().bullet_weight().get_in(WeightGrain)

        cdef float stability_coefficient = 1.0

        cdef calculate_drift = False
        ...
        cdef int ranges_length = int(floor(range_to / step)) + 1
        cdef ranges = []

        cdef float barrel_azimuth = .0
        cdef float barrel_elevation = shot_info.sight_angle().get_in(AngularRadian)
        barrel_elevation = barrel_elevation + shot_info.shot_angle().get_in(AngularRadian)
        cdef float alt0 = atmosphere.altitude().get_in(DistanceFoot)
        print(barrel_elevation)
        cdef float density_factor
        cdef float mach

        # Never used in upstream, uncomment on need
        # density_factor, mach = atmosphere.get_density_factor_and_mach_for_altitude(alt0)

        cdef int current_wind = 0
        cdef float next_wind_range = 1e7

        cdef int len_wind_info = int(len(wind_info))
        ...
        cdef float muzzle_velocity = ammunition.muzzle_velocity().get_in(VelocityFPS)
        cdef gravity_vector = Vector(0, cGravityConstant, 0)
        cdef float velocity = muzzle_velocity
        cdef float time = .0
        cdef drop_adjustment

        # x - distance towards target,
        # y - drop and
        # z - windage

        cdef range_vector = Vector(.0, -weapon.sight_height().get_in(DistanceFoot), 0)
        cdef velocity_vector = Vector(cos(barrel_elevation) * cos(barrel_azimuth), sin(barrel_elevation),
                                      cos(barrel_elevation) * sin(barrel_azimuth)).multiply_by_const(velocity)
        print(velocity_vector)
        cdef int current_item = 0

        cdef float maximum_range = range_to
        cdef float next_range_distance = 0

        cdef float twist_coefficient = .0

        cdef float windage
        cdef windage_adjustment
        cdef float delta_time
        cdef velocity_adjusted
        cdef float drag
        cdef delta_range_vector
