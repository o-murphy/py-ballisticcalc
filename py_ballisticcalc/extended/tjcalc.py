...
class TjCalc(object):
    ...
    def trajectory(self, ammunition: Ammunition, weapon: Weapon, atmosphere: Atmosphere,
        ...
        range_to = shot_info.maximum_distance.get_in(unit.DistanceFoot)
        step = shot_info.step.get_in(unit.DistanceFoot)
        ...
        alt0 = atmosphere.altitude.get_in(unit.DistanceFoot)
        ...
        if len(wind_info) < 1:
            ...
            wind_vector = self.wind_to_vector(shot_info, wind_info[0])
            wind_vector = (wind_vector.x, wind_vector.y, wind_vector.z)

        muzzle_velocity = ammunition.muzzle_velocity.get_in(unit.VelocityFPS)
        # gravity_vector = Vector(0, cGravityConstant, 0)
        gravity_vector = (0, cGravityConstant, 0)
        ...
        range_vector = (.0, -weapon.sight_height.get_in(unit.DistanceFoot), 0)
        ...
        velocity_vector = cvector.multiply_by_const(
            *(math.cos(barrel_elevation) * math.cos(barrel_azimuth),
             math.sin(barrel_elevation),
             math.cos(barrel_elevation) * math.sin(barrel_azimuth)),
            velocity)
        ...
        while range_vector[0] <= maximum_range + calculation_step:
            # if velocity < cMinimumVelocity or range_vector.y < cMaximumDrop:
            if velocity < cMinimumVelocity or range_vector[1] < cMaximumDrop:
                break

            # density_factor, mach = atmosphere.get_density_factor_and_mach_for_altitude(alt0 + range_vector.y)
            density_factor, mach = atmosphere.get_density_factor_and_mach_for_altitude(alt0 + range_vector[1])

            # if range_vector.x >= next_wind_range:
            if range_vector[0] >= next_wind_range:
                current_wind += 1
            ...
            if range_vector[0] >= next_range_distance:
                # windage = range_vector.z
                windage = range_vector[2]
                if calculate_drift:
                ...
                drop_adjustment = self.get_correction(range_vector[0], range_vector[1])
                # windage_adjustment = self.get_correction(range_vector.x, windage)
                windage_adjustment = self.get_correction(range_vector[0], windage)
            ...
            # delta_time = calculation_step / velocity_vector.x
            delta_time = calculation_step / velocity_vector[0]
            # velocity_adjusted = velocity_vector.subtract(wind_vector)
            velocity_adjusted = cvector.subtract(*velocity_vector, *wind_vector)
            # velocity = velocity_adjusted.magnitude()
            velocity = cvector.magnitude(*velocity_adjusted)

            drag = density_factor * velocity * ammunition.bullet.ballistic_coefficient.drag(velocity / mach)
            # velocity_vector = velocity_vector.subtract(
            #     (velocity_adjusted.multiply_by_const(drag).subtract(gravity_vector)).multiply_by_const(delta_time)
            # )
            velocity_vector = cvector.subtract(*velocity_vector, *(
                cvector.multiply_by_const(
                    *cvector.subtract(
                        *cvector.multiply_by_const(
                            *velocity_adjusted, drag
                        ),
                        *gravity_vector
                    ),
                    delta_time
            ...
            # )
            delta_range_vector = (calculation_step, velocity_vector[1] * delta_time, velocity_vector[2] * delta_time)

            # range_vector = range_vector.add(delta_range_vector)
            range_vector = cvector.add(*range_vector, *delta_range_vector)

            # velocity = velocity_vector.magnitude()
            velocity = cvector.magnitude(*velocity_vector)

            # time = time + delta_range_vector.magnitude() / velocity
            time = time + cvector.magnitude(*delta_range_vector) / velocity
