...
PLOT_COLORS: dict[Any, tuple[float, float, float, float]] = {
    ...
    TrajFlag.ZERO_UP: (215 / 255, 155 / 255, 0.0, 1.0),
...
def add_danger_space_overlay(danger_space: DangerSpace, ax: Axes, label: Optional[str] = None) -> None:
    ...
    ax.plot((range_dist, range_dist), (range_drop + h / 2, range_drop - h / 2), color="r", linestyle=":")

    # Shaded danger-space region (filled polygon)
    vertices = ((begin_dist, begin_drop), (end_dist, begin_drop), (end_dist, end_drop), (begin_dist, end_drop))
    ...
    if label != "":  # Add label if not explicitly suppressed
        ax.text(
            begin_dist + (end_dist - begin_dist) / 2,
            end_drop,
            label,
            color="r",
            linespacing=1.2,
...
def add_time_of_flight_axis(ax: Axes, hit_result: HitResult, time_precision: int = 1) -> Axes:
    ...
    sensible_top_labels = [
        time_label_for_distance(x, PreferredUnits.distance, time_precision) for x in sensible_time_ticks
    ]
...
def trajectory_as_plot(hit_result: HitResult, look_angle: Optional[Angular] = None) -> Axes:
    ...
    ax = df.plot(
        x="distance",
        y=["height"],
        ylabel=PreferredUnits.drop.symbol,
        color=PLOT_COLORS["trajectory"],
        linewidth=2,
        ax=ax,
    )
...
def hit_result_as_plot(hit_result, look_angle: Optional[Angular] = None, show_time_axis: bool = True) -> Axes:
    ...
    for p in hit_result.trajectory:
        if TrajFlag(p.flag) & TrajFlag.ZERO:
            ...
            ax.text(
                (p.distance >> PreferredUnits.distance) + max_range / 100,
                df["height"].min(),
        ...
        if TrajFlag(p.flag) & TrajFlag.MACH:
            ax.plot(
                ...
                [df["height"].min(), p.height >> PreferredUnits.drop],
                linestyle=":",
                color=PLOT_COLORS[TrajFlag.MACH],
            )
            ax.text(
                (p.distance >> PreferredUnits.distance) + max_range / 100,
                df["height"].min(),
    ...
    df.plot(
        x="distance",
        xlabel=PreferredUnits.distance.symbol,
        y=["velocity"],
        ylabel=PreferredUnits.velocity.symbol,
        secondary_y=True,
        color=PLOT_COLORS["velocity"],
        ylim=[0, df["velocity"].max()],
        ax=ax,
    )
