...
class AerialTargetPosition(NamedTuple):
    ...
    def __repr__(self):
        preferred = {
            ...
            "look_distance_ft": self.look_distance << PreferredUnits.distance,
...
class AerialTarget:
    ...
    def at_time(self, time_of_flight: float) -> Tuple['AerialTarget', AerialTargetPosition]:
        ...
        distance_vector = Vector(0, math.cos(look_angle_rad), math.sin(look_angle_rad)) * look_distance_ft
        ...
        new_look_distance_ft = (expected_distance_vector.y / math.cos(new_look_angle_rad)) / math.cos(
        ...
        pos = AerialTargetPosition(
            ...
            Distance.Foot(new_look_distance_ft),
    ...
    def get_preemption(self, weapon: Weapon,
        ...
        if not adjust:
            logger.debug(f"t={shot_result.time:.4f}\t"
                         ...
                         f"sd={Unit.Foot((pos.look_distance >> Unit.Foot) * math.cos(pos.look_angle >> Unit.Radian)) >> Unit.Meter:.2f}\t\t\t"
        ...
        initial_look_distance_ft = self._prepared.look_distance_ft
        initial_look_angle_rad = self._prepared.look_angle_rad
        initial_distance_ft = initial_look_distance_ft * math.cos(initial_look_angle_rad)
        ...
        new_distance_ft = (pos_delta.look_distance >> Distance.Foot) * math.cos(pos_delta.look_angle >> Angular.Radian)
        ...
        while True:
            ...
            cur_target_distance_ft = (pos_adjusted.look_distance >> Unit.Foot) * math.cos(
