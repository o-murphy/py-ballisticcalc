...
### Fixed
- `_find_zero_angle()` (`base_engine.py`): Ridder's method checked the angle-step tolerance (`angle_tol`) before computing `f_next`, allowing a premature return when the bracket was narrow but the height error was still larger than `cZeroFindingAccuracy`; fix: compute `f_next` first and verify height accuracy before falling back to the angle-step exit; also introduced a separate `angle_tol = 1e-7 rad` for bracket convergence so it is no longer conflated with `cZeroFindingAccuracy` (feet) — resolves [#204](https://github.com/o-murphy/py-ballisticcalc/issues/204) (scattered `ZeroFindingError` failures on high-elevation targets); local variable renamed `_angle_tol` → `angle_tol` (PEP 8)
- bclibc submodule (`src/engine.cpp`, `tiny_bclibc/include/tiny_bclibc/engine.h`): same Ridder's-method ordering fix applied — `f_next` now evaluated before the `angle_tol` / `kRiddersAngleTol` check in both the C++ and C implementations
