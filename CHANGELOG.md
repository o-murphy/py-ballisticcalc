...
- bclibc submodule (`src/engine.cpp`, `tiny_bclibc/include/tiny_bclibc/engine.h`): same Ridder's-method fix — `f_next` now evaluated before the `angle_tol` / `kRiddersAngleTol` check in both the C++ and C implementations
