...
- `HitResult.get_at()` (`trajectory_data.py`): raised `ArithmeticError: Trajectory does not reach ...` when the requested value was a floating-point epsilon beyond the last trajectory point (e.g. from meter→inch conversion artifacts) instead of returning that last point; the forward-search loop's bracket condition (`curr_val < key_value <= next_val`) excludes any `key_value` past the last point before the `epsilon` check against `traj[-1]` can ever run, so `target_idx` stayed `-1` and the error was raised unconditionally, regardless of how large `epsilon` was passed; fix: check `abs(get_key_val(traj[-1]) - key_value) < epsilon` (and symmetrically `traj[0]`) before raising — resolves [#305](https://github.com/o-murphy/py-ballisticcalc/issues/305); pure-Python fix only — `py_ballisticcalc_exts`' internal `CythonizedBaseTrajSeq.get_at()` (bclibc) uses a different binary-search-and-always-interpolate algorithm that never raises this error in the first place (it silently extrapolates instead), so it isn't affected by this bug
...
- `TestIssue305` added to `tests/test_issues.py` — regression tests for `HitResult.get_at()`: a value a float-epsilon past the last trajectory point resolves to that last point, a value a float-epsilon before the first point resolves to the first point, and a value genuinely far out of range still correctly raises `ArithmeticError`
