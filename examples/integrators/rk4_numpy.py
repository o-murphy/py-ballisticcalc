...
class TrajectoryCalc:
    ...
    def _acceleration(self, velocity_vector: np.ndarray, density_factor: float, mach: float, wind_vector: np.ndarray) -> np.ndarray:
        ...
        return (velocity_adjusted * (-drag_force) + gravity_np) / self.weight # a = F/m (implicitly using weight as mass proxy)
    ...
    def _integrate_rk4(self, shot_info: Shot, maximum_range: float, record_step: float,
        ...
        time = 0.0
        range_vector = np.array([0.0, -self.cant_cosine * self.sight_height, -self.cant_sine * self.sight_height], dtype=np.float64)
        ...
        while range_vector[0] <= maximum_range + min(self.calc_step, record_step):
            ...
            if filter_flags:
                if (data := data_filter.should_record(Vector(*range_vector), Vector(*velocity_vector), mach, time)) is not None:
                    ranges.append(create_trajectory_row(data.time, data.position, data.velocity,
                                                              ...
                                                              density_factor, 0.0, self.weight, data_filter.current_flag)) # Drag is part of acceleration
