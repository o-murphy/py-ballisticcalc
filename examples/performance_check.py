...
from ukrop_338lm_300gr_smk import calc, shot, rk4, rk_4_result
...
# total_time = timeit(lambda: rk4.barrel_elevation_for_target(shot, zero_distance), number=number)
...
#
# total_time = timeit(lambda: rk4.fire(shot, shot_distance, extra_data=False), number=number)
# rate = number / total_time  # executions per second
#
...
# print(f"Execution rate: {rate:.2f} calls per second")
#
# total_time = timeit(lambda: rk4.fire(shot, shot_distance, extra_data=True), number=number)
# rate = number / total_time  # executions per second
#
# print("Calculate trajectory to distance {} {} times:".format(shot_distance, number))
# print(f"Total time: {total_time:.6f} seconds")
